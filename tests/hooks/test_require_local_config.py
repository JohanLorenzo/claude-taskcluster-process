...
@pytest.mark.parametrize(
    "command", ["echo hello", "git status", "taskcluster task log abc"]
)
def test_any_tool_blocked_when_config_missing(tmp_path, command):
    missing = tmp_path / "CLAUDE.local.md"
    allowed, _ = check({"command": command}, local_config_path=missing)
    assert not allowed
